...
Data augmentation techniques---including paraphrase generation, back-translation, and synonym replacement---have improved robustness in low-resource settings \citep{wei2019eda}. More recent work uses LLMs to generate synthetic training data \citep{moller2023prompt}, but such approaches rarely provide \textit{rationales} alongside labels.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{macro_f1.png}
\caption{Macro-F1 across all augmentation conditions and encoder models. LLM augmentation outperforms every non-oracle control by at least 18 points. The golden highlight marks our proposed method.}
\label{fig:macrof1bar}
\end{figure}
...
Table~\ref{tab:perclass} and Figure~\ref{fig:perclassbar} show per-class F1 for BERT-MNLI under the Baseline and LLM conditions.
...
\caption{Per-class F1 for BERT-MNLI under Baseline vs.\ LLM augmentation. The two pragmatically subtle classes (\textit{Prob.\ No}, \textit{In Middle}) rise from zero to $>$0.71 F1 with LLM explanations; other classes are largely unaffected.}
